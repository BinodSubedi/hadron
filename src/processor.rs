...
use crate::routes::get_routes::{get_all, get_any_related, get_custom_filter, get_one};
                ...
                if let Ok(len) = opened.read_to_end(&mut contents) {
                    ...
                    if len == 0 {
                        ...
                        let mut block: GenericArray<u8, U16> =
                            GenericArray::from_slice(&vec_key).clone();

                        cipher.encrypt_block(&mut block);

                        let mut new_file = OpenOptions::new()
                            .append(true)
                            .open(directory.clone() + "/configure.dat")?;
                    ...
                    loop {
                        ...
                        blocks.push(
                            GenericArray::from_slice(&contents[counter..(counter + 16)]).clone(),
                        );
  ...
  /(((((((####################(       .(##########################((/      
    ...
    if args[1].to_string() == String::from("powerup") {
        Ok(rocket::build()
            .mount(
                "/get",
                routes![get_one, get_all, get_any_related, get_custom_filter],
            )
            .mount("/put", routes![put_one])
            .mount("/post", routes![post_one, post_user])
            .mount("/delete", routes![delete_one])
            .mount("/patch", routes![patch]))
    } else {
        process::exit(0);
    }
}
