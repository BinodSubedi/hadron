...
use std::{env, str::FromStr,iter::Iterator};
use std::fs;
...
pub async fn get_any_related(collection:String,number:i8,token:Token) -> Json<GetStandardResponse>{
 ...
 //       println!("{}", collection.to_lowercase());
    ...
    if let Value::Object(obj) = serde_json::from_str(&readFileSchema).unwrap() {
        ...
        if found == None {
            ...
            return Json(GetStandardResponse { status: 400, response: ResponseStatus::NotLinked ,data: vec![Value::Null] });
        ...
        if let Token(Some(token_val)) = token {
           ...
           return Json(
               GetStandardResponse { status: 400, response: ResponseStatus::Failed, data: vec![Value::Null] });
